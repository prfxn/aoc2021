...
private object Day16 {
    ...
    data class Packet(
        val bits: String, val start: Int, val length: Int,
        val subPackets: List<Packet> = listOf(),
        val value: Long = 0
    ) {
        val version get() = getVersion(bits, start)
        val typeId get() = getTypeId(bits, start)
    }
    ...
    fun readLiteralPacket(bits: String, start: Int): Packet {
        ...
        return Packet(
            bits = bits, start = start, length = length,
            value = value
        )
    }
    ...
    fun readOperatorPacketByLength(bits: String, start: Int): Packet {
        ...
        return Packet(
            bits = bits, start = start, length = 7 + 15 + totalLength,
    ...
    fun readOperatorPacketByCount(bits: String, start: Int): Packet {
        ...
        return Packet(
            bits = bits, start = start, length = 7 + 11 + subPackets.sumOf { it.length },
            subPackets = subPackets
        )
    }
